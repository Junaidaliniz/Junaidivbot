...
async def show_panel_ranges(query, panel_name, context):
    ...
    otp_counts = _range_otp_counts.get(panel_name, {})
    otp_normalized = {}
    for k, v in otp_counts.items():
        otp_normalized[k.lower().strip()] = otp_normalized.get(k.lower().strip(), 0) + v
    keyboard = []
    for range_name, nums in grouped.items():
        safe_range = range_name.replace(":", "_")[:30]
        rn_lower = range_name.lower().strip()
        otp_count = otp_normalized.get(rn_lower, 0)
        keyboard.append([
            InlineKeyboardButton(f"\U0001f4c4 {range_name} ({len(nums)})", callback_data=f"range_numbers:{panel_name}:{safe_range}"),
            InlineKeyboardButton(f"\U0001f4e9 {otp_count} OTP", callback_data=f"noop"),
